...
func revokeCert(ctx context.Context, b *HsmPkiBackend, req *logical.Request, serial string, fromLease bool) (*logical.Response, error) {
	...
	if b.pkiBackend.Backend.System().Tainted() {
		return nil, nil
	}
	...
	revEntry, err := pki.FetchCertBySerial(ctx, req, "revoked/", serial)
	if err != nil {
		switch err.(type) {
		case errutil.UserError:
	...
	if !alreadyRevoked {
		certEntry, err := pki.FetchCertBySerial(ctx, req, "certs/", serial)
		if err != nil {
			switch err.(type) {
			case errutil.UserError:
				return logical.ErrorResponse(err.Error()), nil
		...
		if certEntry == nil {
			if fromLease {
				...
				b.pkiBackend.Backend.Logger().Warn("expired certificate revoke failed because not found in storage, treating as success", "serial", serial)
				return nil, nil
			}
		...
		revEntry, err = logical.StorageEntryJSON("revoked/"+pki.NormalizeSerial(serial), revInfo)
		if err != nil {
...
func buildCRL(ctx context.Context, b *HsmPkiBackend, req *logical.Request, forceNew bool) error {
	crlInfo, err := b.pkiBackend.Backend.CRL(ctx, req.Storage)
	if err != nil {
		return errutil.InternalError{Err: fmt.Sprintf("error fetching CRL config information: %s", err)}
	}

	crlLifetime := b.pkiBackend.GetCrlLifetime()
...
WRITE:
	...
	crlBytes, err := signingBundle.Certificate.CreateCRL(rand.Reader, caSigner, revokedCerts, time.Now(), time.Now().Add(crlLifetime))
	if err != nil {
