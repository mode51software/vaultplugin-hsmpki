...
func pathTidy(b *HsmPkiBackend) *framework.Path {
...
func (b *HsmPkiBackend) pathTidyWrite(ctx context.Context, req *logical.Request, d *framework.FieldData) (*logical.Response, error) {
	...
	if !atomic.CompareAndSwapUint32(b.pkiBackend.GetTidyCASGuard(), 0, 1) {
	...
	go func() {
		defer atomic.StoreUint32(b.pkiBackend.GetTidyCASGuard(), 0)

		// Don't cancel when the original client request goes away
		ctx = context.Background()

		logger := b.pkiBackend.Backend.Logger().Named("tidy")
